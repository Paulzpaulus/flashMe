...
@deck_routes.get(
    "/public", response_model=list[DeckRead], summary="Browse all public decks"
)
...
async def get_deck(
    ...
    if not deck.is_public and deck.owner_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="You don't have access to this deck"
        )
...
@deck_routes.post(
    "/", response_model=DeckRead, status_code=201, summary="Create a new deck"
)
...
@deck_routes.put(
    "/{deck_id}", response_model=DeckRead, summary="Update one of your decks"
)
